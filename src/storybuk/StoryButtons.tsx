 ...
 *
 * Horizontal list of buttons for selecting stories.
...
interface StoryButtonsProps {
  ...
  onSelectStory: (demoId: string) => void;
}
...
function DemoButton({
  ...
  // Get concise button label focusing on what makes each story unique
  const getShortName = () => {
...
export function StoryButtons({
  ...
  if (stories.length === 0) {
    return (
      ...
      >
        No stories available for this character
      </div>
    );
  }
    ...
    >
      {stories.map((demo) => (
        <DemoButton
          key={demo.id}
          demo={demo}
          isSelected={selectedStoryId === demo.id}
          onClick={() => onSelectStory(demo.id)}
        />
      ))}
    </div>
  );
