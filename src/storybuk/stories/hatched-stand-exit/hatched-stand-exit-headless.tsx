...
/**
 * Headless Hatched Stand Exit Component
 *
 * Displays state information for the hatched chick stand and exit demo.
 * Shows chick sequence: hatching → standing → exiting.
 */

interface HatchedStandExitHeadlessProps {
  config: ActorConfig;
  resetCount: number;
  shouldStart: boolean;
  canvasWidth: number;
...
export function HatchedStandExitHeadless({
  config,
  resetCount,
  shouldStart,
  canvasWidth,
  canvasHeight,
}: HatchedStandExitHeadlessProps) {
  const actorRef = useRef(
    createActor(hatchedStandExitHeadlessMachine, {
      input: {
        id: config.id || 'hatched-stand-exit-headless',
  ...
  return (
    ...
    >
      ...
      >
        Hatched Stand Exit - Headless Actor
      </h2>
      ...
      >
        ...
        <div style={{ lineHeight: '1.8', color: '#cccccc' }}>
          ...
          <div>2. Hatching (500ms pause, in shell)</div>
          <div>3. Standing (500ms pause, chick standing)</div>
          <div>4. Exiting (1s slide, animated running)</div>
